...
@Time: 2022/3/4 上午11:40
@Author: chh3213
@Description: 优先经验回放，copied from https://github.com/p-christ/Deep-Reinforcement-Learning-Algorithms-with-PyTorch/blob/master/utilities/data_structures/Prioritized_Replay_Buffer.py
========Above the sun, full of fire!=============
"""
import numpy as np
import torch
from module.prioritized_experience_replay.Deque import Deque
from module.prioritized_experience_replay.Max_Heap import Max_Heap
...
class Prioritized_Replay_Buffer(Max_Heap, Deque):
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        raw_td_error = self.give_max_td_error()
    ...
    def separate_out_data_types(self, experiences):
        ...
        states = torch.from_numpy(
            np.vstack([e.value[self.indexes_in_node_value_tuple["state"]] for e in experiences])).float().to(
            self.device)
        actions = torch.from_numpy(
            np.vstack([e.value[self.indexes_in_node_value_tuple["action"]] for e in experiences])).float().to(
            self.device)
        rewards = torch.from_numpy(
            np.vstack([e.value[self.indexes_in_node_value_tuple["reward"]] for e in experiences])).float().to(
            self.device)
        next_states = torch.from_numpy(
            np.vstack([e.value[self.indexes_in_node_value_tuple["next_state"]] for e in experiences])).float().to(
            self.device)
        dones = torch.from_numpy(
            np.vstack([int(e.value[self.indexes_in_node_value_tuple["done"]]) for e in experiences])).float().to(
            self.device)
